...
export class YoutubeApiService {
  private _base_api = `${$API_BASE._api_youtube}/youtube/${$API_BASE._api_youtube_version}/search?key=AIzaSyBkF7QAEjlN88mO5a3rUmREG5DzW6-6_Qo`;
  ...
  _getListYoutube(params: string) {
    return this.http.get<any>(`${this._base_api}&part=snippet&q=${params}&order=viewCount&type=video&videoDefinition=high`);
  }

  _createFavorite(body: IFavorite) {
    let headers: any = new Headers();
    headers.append('Authorization', `Bearer ${localStorage.getItem('accessToken')}`);
    return this.http.post<any>(`${this._base_api_firestore}/createFavorite`, body,{ headers: headers});
  }
